resource "google_compute_health_check" "health_check" {
  for_each = var.healthcheck_config != null && var.lb_type.is_external && var.lb_type.is_proxied ? var.healthcheck_config : {}
  name                = "hc-${var.name}"
...
resource "google_compute_region_health_check" "health_check" {
  for_each = var.healthcheck_config != null && !(var.lb_type.is_global) && !(local.is_region_external_proxy_lb) ? var.healthcheck_config : {}  
  name                = "hc-${var.name}"
  project             = var.project_id
  region = var.region
  description         = each.value.description
